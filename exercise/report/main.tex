...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\linewidth]{Images/cifar10-samples.png}
	\vspace{10pt}
...
\subsection{So sánh tổng hợp 5 mô hình (Phần 1--3)}

Biểu đồ dưới đây so sánh Validation Accuracy và Validation Loss của 5 mô hình đã trình bày: Softmax, MLP, CNN, ViT (PyTorch API) và Custom ViT.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Images/val-comparison-5models.png}
	\vspace{10pt}
	\caption{So sánh Validation Accuracy và Loss của Softmax, MLP, CNN, ViT và Custom ViT qua các epoch}
	\label{fig:val-comparison-5models}
\end{figure}
...
\subsection{Biểu đồ so sánh tất cả các mô hình}

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Images/val-comparison-all-models.png}
	\vspace{10pt}
	\caption{So sánh Validation Accuracy và Loss của tất cả 6 mô hình (Softmax, MLP, CNN, ViT, Hybrid CNN-ViT, Overlapping ViT) cùng bảng Test Accuracy}
	\label{fig:all-models-comparison}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Images/val-comparison-all-models-2.png}
	\vspace{10pt}
	\caption{So sánh chi tiết Validation Accuracy và Loss giữa tất cả các mô hình qua các epoch}
	\label{fig:all-models-comparison-2}
\end{figure}
