...
\begin{itemize}
	...
	\item \textbf{GFLOPs (Giga Floating-point Operations Per Second):} Thống kê quy mô hàng tỷ phép toán phẩy động mô hình học sâu cần thực thi để phân tích xong 1 file ảnh. Tích số GFLOPs càng nhỏ gọn thể hiện kiến trúc mạng càng tối giản, ít tiêu hao điện năng (Low-power consumption).
\end{itemize}
...
\textbf{Insight Phân tích Số liệu Đối chiếu (mAP):}
\begin{itemize}
	\item \textbf{Luận điểm đánh đổi (Trade-off):} Dựa trên bảng số liệu mAP, sức mạnh trích xuất đặc trưng của kiến trúc Two-stage được thể hiện đúng như lý thuyết. Faster R-CNN đạt mAP@0.5 toàn tập là \textbf{0.915}, vượt trội hơn mốc \textbf{0.903} của YOLOv8m. Khoảng cách này càng được kéo giãn ở thước đo khắt khe \textbf{mAP@0.5:0.95} (0.702 so với 0.671).
	\item \textbf{Xử lý nhiễu nội biên (Small Object \& Occlusion):} Nhờ bộ FPN đa độ phân giải kết hợp RoI Align giữ lại cấu trúc vi mô, Faster R-CNN áp đảo ở các đối tượng khó, thiểu số. Đơn cử ở nhãn \texttt{Pedestrian} và \texttt{Person\_sitting}, Faster R-CNN đem lại độ cải thiện ấn tượng (lần lượt tăng lên 0.865 và 0.781) so với mức suy giảm (0.834 và 0.726) của YOLOv8m. Lưới thiết kế "Anchor-free" của YOLOv8 kết hợp nén dữ liệu MixUp là một nỗ lực lớn nhưng chưa khắc phục tuyệt đối điểm mù nhòe ảnh (Motion Blur).
	\item \textbf{Nhóm phương tiện lớn:} Cả hai mô hình chia sẻ điểm tương đồng tuyệt đối với phân tích hình thái (EDA). Các vật thể to bản như \texttt{Car}, \texttt{Truck} dễ dàng được định vị, đạt mAP@0.5 cùng tiệm cận độ hoàn hảo (trên 96.5\%).
\end{itemize}
...
\begin{enumerate}
	\item \textbf{Đánh giá đo lường FPS và Tính ứng dụng công nghiệp:} Ở các tác vụ xe tự hành trên cao tốc, ngưỡng $30\text{ FPS}$ là vạch an toàn tối thiểu và bắt buộc để hệ thống rà quét phanh điện tử. Theo số liệu, \textbf{Faster R-CNN} chỉ đạt \textbf{7.0 FPS} (độ trễ lớn khoảng 140ms để đọc xong một khung hình). Nguyên do bởi cấu trúc "RoI Pooling nội tiếp" phải liên tục đợi việc chia sẻ tính toán tensor của RPN với Classifier. Chậm trễ này khiến hệ thống phản ứng kém thời gian thực, có nguy cơ gây va chạm trước khi mô hình xử lý lệnh rẽ. Ngược lại, \textbf{YOLOv8m} đáp ứng tốt tiêu chuẩn thời gian thực với \textbf{45.4 FPS}.
