...
\begin{enumerate}
    ...
    \item \textbf{Full fine-tuning không vượt trội} (64.4\%), thậm chí còn kém hơn zero-shot. Nguyên nhân có thể do:
    \begin{itemize}
        \item Dataset size nhỏ (chỉ 43 classes với 100 samples/class)
        \item Overfitting khi unfreeze toàn bộ backbone
        \item CLIP pretrained đã quá mạnh cho COCO, không cần fine-tune thêm
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Full fine-tuning không luôn tốt hơn}: Với dataset nhỏ (43 classes, 100 samples/class), việc unfreeze backbone không mang lại cải thiện, thậm chí còn kém hơn zero-shot do overfitting.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Prompt engineering}: Thử các template prompt phức tạp hơn cho zero-shot
